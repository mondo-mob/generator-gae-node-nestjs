import { createLogger } from '@mondomob/gae-node-nestjs';
...
export class SecretsResolver {
  ...
  async resolveSecrets<T extends object>(obj: T): Promise<T> {
  ...
  private async resolvePairWithObject([key, value]: [string, object]): Promise<[string, object]> {
