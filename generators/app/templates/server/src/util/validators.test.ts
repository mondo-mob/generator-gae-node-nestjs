...
describe('validators', () => {
  describe('validateFound', () => {
    ...
    it('throws not found when null', () => {
      expect(() => validateFound('MyType', 'myId', null)).toThrow(
        new NotFoundException('MyType not found for identifier: myId'),
      );
    });
  });
  ...
  describe('validate', () => {
    ...
    it('throws validation error when expression is false', () => {
      expect(() => validate(500 < 1, "That ain't right")).toThrow(new BadRequestException("That ain't right"));
    });
  ...
  describe('validateNotNil', () => {
    ...
    it('throws validation error when null', () => {
      expect(() => validateNotNil(null, "Can't be nil")).toThrow(new BadRequestException("Can't be nil"));
    });

    it('throws validation error when undefined', () => {
      expect(() => validateNotNil(undefined, "Can't be nil")).toThrow(new BadRequestException("Can't be nil"));
    });
  });
  ...
  describe('validateArrayNotEmpty', () => {
    ...
    it('throws validation error when empty array', () => {
      expect(() => validateArrayNotEmpty([], "Can't be empty")).toThrow(new BadRequestException("Can't be empty"));
    });

    it('throws validation error when null', () => {
      expect(() => validateArrayNotEmpty(null, "Can't be empty")).toThrow(new BadRequestException("Can't be empty"));
    });

    it('throws validation error when undefined', () => {
      expect(() => validateArrayNotEmpty(undefined, "Can't be empty")).toThrow(
        new BadRequestException("Can't be empty"),
      );
    });
  });
});
